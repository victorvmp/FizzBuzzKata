...
class FizzBuzzTest {
    ...
    fun test() {
        ...
        val result = FizzBuzz().convert(IntRange(1, 100))

        assertEquals(expected, result)
    }

    fun assertEquals(expected: List<Any>, actual: List<Any>) {
        expected.forEachIndexed { position, it -> assert(it.equals(actual[position]), { print("At position $position -> Expected : $it , Actual: ${actual[position]}") }) }
    }
}
